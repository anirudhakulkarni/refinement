...
def parse_option():
    ...
    opt.model_name = 'basic_{}_{}_{}_{}_im_{}_lr_{}_decay_{}_bsz_{}_d_{}_temp_{}_trial_{}_epochs_{}_optim_{}_prefix_{}_freeze_{}'.\
...
def set_model(opt):
    ...
    if opt.loss == 'ce':
        ...
        criterion = FocalLoss(gamma=opt.gamma,alpha=opt.alpha)
    elif opt.loss == 'mdca':
        criterion = ClassficationAndMDCA(beta=opt.beta)
    else:
        criterion = torch.nn.CrossEntropyLoss()
    # criterion = loss_dict['LogitNorm']()
    # criterion = ClassficationAndMDCA(beta=0.1)
...
def main(opt):
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        time1 = time.time()
        # train for one epoch
        results = train(train_loader2, model, classifier, criterion, optimizer, epoch, opt)
        time2 = time.time()
        ...
        # eval for one epoch
        results = validate(val_loader, model, classifier, criterion, opt)
        # log_results(logger, results, epoch)
        log_results(results, epoch)
