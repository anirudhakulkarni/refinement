from libauc.losses import AUCMLoss, AUCM_MDCALoss
...
# dataloader
train_data, train_targets = STL10(
    root='../../MDCA-Calibration-main/data', split='train')
test_data, test_targets = STL10(
    root='../../MDCA-Calibration-main/data', split='test')
print("data found")
...
# train_images, train_labels = train_data.astype(
#     np.uint8), train_targets.astype(np.uint8)
# test_images, test_labels = test_data.astype(
#     np.uint8), test_targets.astype(np.uint8)
...
for gamma in gammas:
    for margin in margins:
        ...
        # save results
        with open(lossfunction + "_results_stl_"+str(SEED)+".txt", "a+") as f:
