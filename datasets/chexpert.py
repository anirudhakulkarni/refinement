...
import pandas as pd
# https://github.com/Optimization-AI/ICCV2021_DeepAUC/blob/main/chexpert.py
...
class CheXpert(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.class_index != -1:  # multi-class mode
            ...
            # label = np.array(self._labels_list[idx]).reshape(-1).astype(np.int)
            label = np.array(self._labels_list[idx]).astype(np.int)
            # label = np.array(np.where(label == 1))
            # print(label)
        else:
            label = np.array(
                self._labels_list[idx]).reshape(-1).astype(np.float32)
            # label = np.array(self._labels_list[idx]).reshape(-1)
            # print(label)
            # label = np.array(np.where(label == 1))
            # print(label)
        if("CRL" in self.args.loss):
...
def get_train_valid_test_loader(args):
    ...
    testloader = torch.utils.data.DataLoader(
        testSet, batch_size=args.test_batch_size, num_workers=args.workers, drop_last=False, shuffle=False)
    trainloader = torch.utils.data.DataLoader(
        traindSet, sampler=train_sampler, batch_size=args.train_batch_size, num_workers=args.workers, drop_last=False)
    valloader = torch.utils.data.DataLoader(
        traindSet, sampler=val_sampler, batch_size=args.test_batch_size, num_workers=args.workers, drop_last=False)
